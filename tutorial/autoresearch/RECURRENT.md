...
> ⚠️ **정정 (COMPARE3.md)**: 위 "병렬"은 *naive 구현*(L×L materialize, O(L²))이라 16384 OOM이었다.
> 이는 구현 결함 → **chunked(GLA 표준)로 재구현해 O(L) 해결**(16384=321MB, OOM 없음, FlashAttn
> 트랜스포머 371MB보다도 적음). 정확도는 chunked=naive=재귀 동치(5e-7).
> 추론 메모리 이점은 그대로: 트랜스포머 KV캐시 O(L)(16384=128MB) vs 재귀 상태 O(1)(132KB) ≈ **992×**.
