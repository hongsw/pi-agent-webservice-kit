...
> ⚠️ **정정/맥락 (COMPARE3.md 참조)**: 위 "병렬"은 *우리의 naive 선형어텐션 구현*(L×L 감쇠행렬을
> materialize)이라 O(L²)다. 표준 **트랜스포머는 FlashAttention(PyTorch SDPA)으로 O(L)**라 16384에서
> OOM이 나지 않는다(371MB). 따라서 *학습 메모리* 우위는 우리 구현으론 성립하지 않는다.
> 메모리캐싱의 진짜 이점은 **자기회귀 추론**: 트랜스포머 KV캐시 O(L)(16384=128MB) vs 재귀 상태
> O(1)(132KB) ≈ **992×**, 길이 무관. 3자 실측은 [COMPARE3.md](COMPARE3.md).
