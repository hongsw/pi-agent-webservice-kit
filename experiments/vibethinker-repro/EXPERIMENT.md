...
### S1 — AIME25 재현 (완료, bf16, 4090)
- 설정: `WeiboAI/VibeThinker-3B` bf16, AIME2025 30문항(yentinglin/aime_2025), temp=1.0/top_p=0.95,
  **max_new=16384**(자원 절감), pass@1(단일샘플). 82분.
- **결과: pass@1 = 66.7% (20/30)**, 평균 12,137토큰, **truncation율 53.3%**.
- **격차 분해(핵심)**: claim(CLR미적용) 91.4 대비 약 25%p 낮으나 —
  - **잘리지 않은 문항: 14/14 = 100% 정답** / 잘린 문항: 6/16(37.5%).
  - 즉 16k 안에 추론을 끝내면 **전부 정답**. 격차는 모델 능력이 아니라 **토큰 한도**(claim은 max 40,960).
- **R2 판정**: 사전등록 기준 10~30%p 낮음 → **"부분재현(설정차)"**. 설정차 = max_new(16k vs 40k) + 단일샘플.
- **개선 방향(R3) 시사**: VibeThinker의 Long2Short(적은 토큰으로 풀기)가 바로 이 truncation 병목을 겨냥 →
  S2에서 (a) max 40,960 재측정으로 상한 확인, (b) Long2Short/효율 개선 탐색.
